...
import 'package:media_kit_video/src/controller/media_kit_android_video_controller.dart';
import 'package:media_kit_video/src/controller/media_kit_native_video_controller.dart';
import 'package:media_kit_video/src/controller/media_kit_video_platform_controller.dart';
import 'package:media_kit_video/src/model/media_kit_video_configuration.dart';

/// {@template media_kit_video_controller}
///
/// MediaKitVideoController
/// ---------------
///
/// [MediaKitVideoController] is used to initialize & display video output.
/// It takes reference to existing [Player] instance from `package:media_kit`.
///
/// Passing [MediaKitVideoController] to [MediaKitVideoView] widget will cause the video output to be displayed.
///
/// ```dart
/// late final player = Player();
/// late final controller = MediaKitVideoController(player);
/// ```
///
...
/// **Web**
/// * [VideoControllerConfiguration.width] & [VideoControllerConfiguration.height] arguments have no effect.
/// * Only single [Video] output can be displayed for a [MediaKitVideoController].
///   Displaying multiple [Video] widgets with same [MediaKitVideoController] will cause only last mounted [Video] to be displayed.
...
class MediaKitVideoController {
  /// The [Player] instance associated with this [MediaKitVideoController].
  final Player player;

  /// Platform specific internal implementation initialized depending upon the current platform.
  final platform = Completer<MediaKitVideoPlatformController>();

  /// Platform specific internal implementation initialized depending upon the current platform.
  final notifier = ValueNotifier<MediaKitVideoPlatformController?>(null);
  ...
  MediaKitVideoController(
    this.player, {
    MediaKitVideoConfiguration configuration =
    ...
    () async {
      ...
      try {
        if (MediaKitNativeVideoController.supported) {
          final result = await MediaKitNativeVideoController.create(
            player,
            configuration,
          );
        ...
        } else if (MediaKitAndroidVideoController.supported) {
          final result = await MediaKitAndroidVideoController.create(
            player,
            configuration,
          );
        ...
        if (platform.isCompleted) {
          ...
          platform.completeError(
            UnimplementedError(
              '[MediaKitVideoController] is unavailable for this platform.',
            ),
          );
        }
